...
class GoodsLocationControllerSpec extends SpecBase with MockitoSugar with NunjucksSupport with JsonMatchers {
  ...
  lazy val goodsLocationRoute = routes.GoodsLocationController.onPageLoad(NormalMode).url
  ...
  "GoodsLocation Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(GoodsLocationPage, GoodsLocation.values.head).success.value
